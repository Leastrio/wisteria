...
impl SysInfoService {
  pub async fn run(self: Arc<Self>, executor: BackgroundExecutor) {
    ...
    loop {
      ...
      let cpu_temp = components.iter().find(|c| c.label() == "k10temp Tctl").and_then(|c| c.temperature()).unwrap_or(0.0);
      let coolant_temp = components.iter().find(|c| c.label() == "d5next Coolant temp").and_then(|c| c.temperature()).unwrap_or(0.0);
      let ram_used = sys.used_memory() / 1024 / 1024 / 1024;
      let disk_usage = disks.iter().find(|d| d.mount_point() == Path::new("/")).map(|d| {
      ...
      self.stats.set(SysStats { cpu_usage, cpu_temp, coolant_temp, ram_used, disk_usage });
